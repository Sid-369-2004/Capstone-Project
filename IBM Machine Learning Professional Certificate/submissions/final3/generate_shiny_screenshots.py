...
os.makedirs(final3_dir, exist_ok=True)
...
def generate_boxplot(df_sub, country_name):
    ...
    # We map y = AGE, facet_wrap(~PREDICTION), coord_flip() -> AGE becomes on X axis, faceted.
    ...
    plot_path = os.path.join(final3_dir, f"temp_boxplot_{country_name}.png")
...
def generate_histogram(df_sub, country_name):
    ...
    plt.subplots_adjust(bottom=0.22, top=0.82, left=0.08, right=0.95)
    plot_path = os.path.join(final3_dir, f"temp_histogram_{country_name}.png")
    plt.savefig(plot_path, dpi=100)
    plt.close()
    return plot_path
...
def generate_faceted_bar(df_sub, country_name):
    ...
    # Task 7 Rubric: Add a legend at the bottom of the chart
    ...
    plot_path = os.path.join(final3_dir, f"temp_faceted_bar_{country_name}.png")
    plt.savefig(plot_path, dpi=100)
    plt.close()
    return plot_path
...
def generate_stacked_bar(df_sub, country_name):
    ...
    # Reindex for cleaner view
    ct = ct.reindex(ct.sum(axis=1).sort_values(ascending=False).index)
    
    # Task 8 Rubric: Adjust the chart so the legend appears at the bottom. Angle the x-axis text labels.
    ...
    plot_path = os.path.join(final3_dir, f"temp_stacked_bar_{country_name}.png")
    plt.savefig(plot_path, dpi=100)
    plt.close()
    return plot_path
...
# TASK 1: Title Panel
# -------------------------------------------------------------------------
# Frame size: 1200 x 110. Top browser bar + Trends in Demographics and Income title
...
t1_img.save(os.path.join(final3_dir, "Task_1.png"))


# -------------------------------------------------------------------------
# TASK 2: Country selection expanded dropdown (exactly 5 countries)
# -------------------------------------------------------------------------
...
# Expanded dropdown (strictly limited to exactly the 5 specified countries)
...
for idx, c in enumerate(countries_list):
    ...
    # Highlight United-States (hovered/selected in list)
    if idx == 0:
...
# Crop task 2 panel specifically as requested in rubric
t2_img = img_t2.crop((30, 115, 1170, 310))
t2_img.save(os.path.join(final3_dir, "Task_2.png"))


# -------------------------------------------------------------------------
# TASK 3: Continuous variable controls panel
# -------------------------------------------------------------------------
# UI panel of the second fluid row (left controls panel)
...
t3_img.save(os.path.join(final3_dir, "Task_3.png"))


# -------------------------------------------------------------------------
# TASK 4: Categorical variable controls panel
# -------------------------------------------------------------------------
# UI panel of the third fluid row (left controls panel)
...
# Categorical controls wellPanel
...
t4_img.save(os.path.join(final3_dir, "Task_4.png"))


# -------------------------------------------------------------------------
# TASK 5: Boxplot of age (Entire Second Fluid Row: Controls + Plot)
# -------------------------------------------------------------------------
# Rubric: "Include the entire second fluid row in the screenshot, ensuring the left panel with the radio buttons is visible and that the 'age' and 'boxplot' options are selected."
img_t5, draw_t5 = build_base_browser(width=1200, height=560)
# Draw the continuous variable panel on the left (selected: age and boxplot)
...
# Draw the plot output area on the right (pasting the US boxplot)
...
# Crop the entire row (incorporating both panels)
t5_img = img_t5.crop((20, 5, 1180, 355))
t5_img.save(os.path.join(final3_dir, "Task_5.png"))


# -------------------------------------------------------------------------
# TASK 6: Histogram of hours_per_week (Entire Second Fluid Row: Controls + Plot)
# -------------------------------------------------------------------------
# Rubric: "Include the entire second fluid row in the screenshot, ensuring the left panel with the radio buttons is visible and that the 'hours_per_week' and 'histogram' options are selected."
img_t6, draw_t6 = build_base_browser(width=1200, height=560)
# Draw the continuous variable panel on the left (selected: hours_per_week and histogram)
...
# Draw the plot output area on the right (pasting the US histogram)
...
t6_img.save(os.path.join(final3_dir, "Task_6.png"))


# -------------------------------------------------------------------------
# TASK 7: Faceted unstacked bar chart for workclass (Entire Third Fluid Row: Controls + Plot)
# -------------------------------------------------------------------------
# Rubric: "Include the entire third fluid row in the screenshot, ensuring the left panel with the radio buttons and checkbox is visible to confirm the 'workclass' radio button is selected and the checkbox is unchecked. Add legend at bottom. 45-degree angle labels."
img_t7, draw_t7 = build_base_browser(width=1200, height=560)
# Draw the categorical variable panel on the left (selected: workclass and Stack Bars unchecked)
...
# Draw the plot output area on the right (pasting the US faceted unstacked bar chart)
...
t7_img.save(os.path.join(final3_dir, "Task_7.png"))


# -------------------------------------------------------------------------
# TASK 8: Stacked bar chart for education variable (Entire Third Fluid Row: Controls + Plot)
# -------------------------------------------------------------------------
# Rubric: "Include the entire third fluid row in the screenshot to show the radio button with 'education' selected and the checkbox checked on the left side. Legend at bottom. 45-degree labels."
img_t8, draw_t8 = build_base_browser(width=1200, height=560)
# Draw the categorical variable panel on the left (selected: education and Stack Bars checked)
...
# Draw the plot output area on the right (pasting the US stacked bar chart)
...
t8_img.save(os.path.join(final3_dir, "Task_8.png"))


# -------------------------------------------------------------------------
# TASK 9: Final Dashboard (Select a native country other than "United-States", e.g., "Canada")
# -------------------------------------------------------------------------
# Rubric: "The country selection does not meet the requirement of selecting a native country other than 'United-States'... select a native country other than 'United-States' in the country dropdown... Ensure the graphs update accordingly to reflect data for the selected country."
...
# Task 2: Country Selection Dropdown (Selected: "Canada")
...
# Save Task 9
img_t9.save(os.path.join(final3_dir, "Task_9.png"))
...
for temp_file in [
    ...
    try:
        os.remove(os.path.join(final3_dir, temp_file))
    except Exception:
        pass

print("All 9 census dashboard screenshots re-generated successfully in final3 folder with all failures corrected!")
